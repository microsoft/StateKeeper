...
/// <summary>
/// Function the grain invokes to let test code assert that execution is still happening
/// in the grain's task scheduler.
/// </summary>
public delegate void CheckTaskScheduler();

/// <summary>
/// An async test callback that runs inside a grain and receives a
/// <see cref="CheckOrleans"/> to verify scheduler affinity.
/// </summary>
public delegate Task GrainTestFunction(CheckOrleans checkOrleans, CheckTaskScheduler checkTaskScheduler);
